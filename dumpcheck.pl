# Monitor backup files, most typically database dumps for time and significant changes in size.
...
$command=qq{$ENV{XYMONCLIENTHOME}/bin/xymon $ENV{XYMSRV} "status$CONFIG{status_duration} $ENV{MACHINE}.$CONFIG{CHECKNAME} $statusColor $commandDate $statusMessage"};
if( -s "$ENV{XYMON}" ) {
	`$command`;
	if( $status ) {
